...
class theme_marble_core_renderer extends core_renderer {
	
	
	public function user_menuB() {
        global $CFG;
    ...
    protected function marble_process_message($message, $state) {
        global $DB;
...
//end usermenu
    ...
    protected function render_custom_menu(custom_menu $menu) {
		...
		//turn of my courses
		    $homer = (!empty($this->page->theme->settings->homer));
		    if ($homer == "") { 
		    $homer = "Home";
		    } else {
		    $homer = $this->page->theme->settings->homer;
		    }
		     $marbleursetitle = $homer;
             $branchtitle = $homer;
		   //$marbleursetitle = get_string('homelang', 'theme_marble');
           //$branchtitle = get_string('homelang', 'theme_marble');
		...
		if (isloggedin() && !isguestuser() && $branchurlB == "test") {
		
	
		
        	$marbleursetitle = "My Courses";
           $branchtitle ="My Courses";
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_marble');
    ...
    public function marbleblocks($region, $classes = array(), $tag = 'aside') {
...
class theme_marble_core_course_renderer extends core_course_renderer {
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        $content .= html_writer::start_tag('div', array('class' => 'span4 panel panel-default coursebox triggerAnimation animated', 'data-animate' => 'flipInY'));
        ...
        $content .= $this->coursecat_coursebox_content2($chelper, $course);
    ...
    protected function coursecat_coursebox_content2(coursecat_helper $chelper, $course) {
        global $CFG;
        // if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
        //     return '';
        // }
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            if ($isimage) {
               ...
               $contentimages .= "<div class='cimbox' style='background:url($url) no-repeat 50% 0; background-size: cover;'>
               <div class='hover-box'>
									
									<a class='link' href='$urlb'><i class='fa fa-link'></i></a>
								</div>
        ...
        if ($contentimages == "") {
        $urlb = new moodle_url('/course/view.php', array('id' => $course->id));
        $contentimages .= "<div class='cimbox noim' style=''>
        <div class='hover-box'>
		...
		//moved course name
		$coursename = $chelper->get_course_formatted_name($course);
		$content .= "<a class='link' href='$urlb'><h3>";
		$content .=  html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)),
        $coursename, array('class' => $course->visible ? '' : 'dimmed'));
    ...
    public function course_search_form2($value = '', $format = 'plain') {
        static $count = 0;
        $formid = 'coursesearch';
        if ((++$count) > 1) {
            $formid .= $count;
